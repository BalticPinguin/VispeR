...
def errFCfOPA(J, K, f, Energy, N, T,E0):
   ...
   *RETURNS:*
   ...
   def CalcI00(dim, E):
	 ...
	 zeros[i]=0
      linspect.append(np.matrix([10, E*Hartree2cm_1, 0 ]))
      return opa
   ...
   def makeLine2(intens,E0, T, index, ex, Gamma, Gammap,E, n):
   ...
   for i in range(1,N+1):
      ...
      linspect.append(makeLine(intens,E0, T, index, excitation, Gamma, Gammap, Energy, i))
   dimen=0
   ...
   for i in range(1,len(linspect)):
      spect[len(linspect[i-1]):len(linspect[i-1])+len(linspect[i])]=linspect[i]
   return spect.T
...
def oldFCfOPA(J, K, f, Energy, N, T,E0):
   """Calculates the FC-factors for given Duschinsky-effect. No restriction to OPA
